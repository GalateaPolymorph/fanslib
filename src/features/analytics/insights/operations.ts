...
export const generateHashtagInsights = (posts: FanslyPostWithAnalytics[]): HashtagInsight[] => {
  ...
  hashtagMap.forEach((data, hashtag) => {
    if (data.posts.length < 3) return; // Need minimum sample size
  ...
  return insights.sort((a, b) => b.confidence - a.confidence).slice(0, 3); // Top 3 hashtag insights
};
...
export const generateContentThemeInsights = (
  ...
  posts.forEach((post) => {
    ...
    // Extract meaningful words (excluding hashtags and common words)
    ...
    // Group by dominant theme based on word frequency
    words.forEach((word) => {
  ...
  themeMap.forEach((data, theme) => {
    if (data.posts.length < 4) return; // Need minimum sample size
  ...
  return insights.sort((a, b) => b.confidence - a.confidence).slice(0, 2); // Top 2 theme insights
};
...
export const generatePostTimingInsights = (
  ...
  const timeSlotData = Array.from(timeSlotMap.entries())
    ...
    .filter((slot) => slot.postCount >= 2); // Minimum sample size per slot
  ...
  if (performanceBoost < 10) return null; // Only recommend if significant improvement
