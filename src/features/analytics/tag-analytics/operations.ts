...
export const getTagAnalytics = async (
  ...
  return Object.values(tagMetrics).map((metric) => ({
    ...
    averageEngagement: metric.totalAssignments > 0 ? metric.totalEngagement / metric.totalAssignments : 0,
    topPerformingMedia: metric.mediaIds.slice(0, 5),
    trendDirection: metric.recentPerformance > metric.totalEngagement / metric.totalAssignments ? "up" : "down",
...
export const getTagCorrelations = async (dimensionId?: number): Promise<TagCorrelationData[]> => {
  ...
  const queryBuilder = dataSource
    ...
    .innerJoin(MediaTag, "mt2", "mt1.mediaId = mt2.mediaId AND mt1.tagDefinitionId < mt2.tagDefinitionId")
  ...
  const correlationMap = correlations.reduce((acc, mt) => {
...
export const getTagTrends = async (tagIds: number[], timeRange: { start: Date; end: Date }) => {
  ...
  const trendData = trends.reduce((acc, mt) => {
    const tagId = mt.tagDefinitionId;
    if (!acc[tagId]) {
      acc[tagId] = {
        tagId,
