...
const loadingSkeletonVariants = cva("animate-pulse bg-muted rounded", {
  variants: {
    variant: {
      default: "bg-muted",
      card: "bg-muted",
...
const SkeletonText = ({
  ...
  className,
  ..._props
}: { lines?: number } & Omit<LoadingSkeletonProps, "variant">) => (
...
const SkeletonAvatar = ({
  ...
  className,
  ...props
}: { size?: "sm" | "default" | "lg" } & Omit<LoadingSkeletonProps, "variant" | "size">) => {
  const sizeClasses = {
    sm: "w-8 h-8",
    default: "w-10 h-10",
    lg: "w-12 h-12",
  ...
  return (
    <LoadingSkeleton variant="avatar" className={cn(sizeClasses[size], className)} {...props} />
  );
};
